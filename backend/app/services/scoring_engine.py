...
class ScoringEngine:
    ...
    def _compute_shap_explanations(self, features: Dict[str, float], feature_vector: np.ndarray) -> List[Dict]:
        ...
        if not explanations:
            heuristics = [
                ("revenue_growth_3m", 0.15, "positive", lambda f: f"Revenue grew {abs(f):.0f}% over the last 3 months — strong positive signal" if f > 15 else f"Revenue declined {abs(f):.0f}% over the last 3 months — negative signal"),
                ("customer_concentration_hhi", -0.12, "negative", lambda f: f"{f*100:.0f}% of revenue from a single customer increases risk" if f > 0.5 else "Customer base is well diversified"),
                ("working_capital_ratio", 0.10, "positive", lambda f: "Healthy cash inflow to outflow ratio indicates good liquidity" if f > 1.1 else "Cash outflow exceeds inflow — working capital stress"),
                ("circular_flow_score", -0.18, "negative", lambda f: "Suspicious circular transaction patterns detected" if f > 0.3 else "No circular transaction patterns detected"),
                ("business_age_months", -0.08, "negative", lambda f: "Business is relatively new — higher risk profile" if f < 18 else "Established business with operational history"),
                ("mca_compliance_score", 0.08, "positive", lambda f: "Strong regulatory compliance history" if f > 0.7 else "Compliance issues detected in MCA filings"),
                ("gst_filing_timeliness_score", 0.06, "positive", lambda f: "Regular and timely GST filing pattern" if f > 0.7 else "Irregular GST filing pattern"),
                ("invoice_regularity_score", 0.05, "positive", lambda f: "Regular invoice generation indicates stable operations" if f > 0.5 else "Irregular invoice pattern"),
            ]
            
            for feature_name, default_impact, default_dir, text_fn in heuristics:
